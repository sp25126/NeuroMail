...
def callback(
    ...
    try:
        ...
        # Fetch email address from Microsoft Graph to store in scope_state for mapping webhook notifications
        import requests
        try:
            ...
            if profile_res.status_code == 200:
                ...
                email_address = profile_json.get("mail") or profile_json.get("userPrincipalName")
                if email_address:
                    mailbox.scope_state = email_address
        except Exception as profile_err:
            logger.warning(f"Failed to fetch Outlook user profile: {str(profile_err)}")
            # Fall back to default if profile call fails
            if not mailbox.scope_state:
                mailbox.scope_state = "outlook_mailbox"
        ...
        raise HTTPException(
            ...
            detail=f"Outlook authentication failed: {str(e)}"
        )
