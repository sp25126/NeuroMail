...
async function searchGmail(query: string) {
    try {
        ...
        const threadDetails = await Promise.all(
            (threadsRes.data.threads || []).map(async (t: any) => {
                try {
                    ...
                    return {
                        ...
                        from: lastMsg?.payload?.headers?.find((h: any) => h.name === "From")?.value,
                        to: lastMsg?.payload?.headers?.find((h: any) => h.name === "To")?.value,
                        subject: lastMsg?.payload?.headers?.find((h: any) => h.name === "Subject")?.value,
                        date: lastMsg?.payload?.headers?.find((h: any) => h.name === "Date")?.value,
                    };
                } catch { return null; }
            })
        );
