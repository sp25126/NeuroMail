    ...
    return (
        <div className="flex flex-col h-full bg-background overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
                ...
                {/* TrackFlow AI Extraction Section */}
                {!isLoadingFreightConfig && (
                    <motion.section
                        initial={{ opacity: 0, y: 15 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="font-bold text-foreground">AI Extraction Engine</h2>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] font-bold text-muted-foreground uppercase">Enable AI Fallback</span>
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={freightConfig.ai_extraction_enabled}
                                        onChange={(e) => setFreightConfig({ ...freightConfig, ai_extraction_enabled: e.target.checked })}
                                        className="sr-only peer"
                                    />
                                    <div className="w-9 h-5 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white/60 after:border after:border-transparent after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary" />
                                </label>
                        ...
                        {freightConfig.ai_extraction_enabled ? (
                            <div className="space-y-4 animate-in fade-in duration-300">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-semibold text-muted-foreground mb-1.5 uppercase tracking-wider">Primary Model</label>
                                        <select
                                            value={freightConfig.primary_ai_model}
                                            onChange={(e) => setFreightConfig({ ...freightConfig, primary_ai_model: e.target.value })}
                                            className="w-full px-3 py-2 text-sm bg-white/5 border border-white/10 rounded-lg text-foreground outline-none focus:ring-2 focus:ring-primary/50 transition-all"
                                        >
                                            <option value="gpt-4o">GPT-4o (OpenAI)</option>
                                            <option value="claude-3-5-sonnet">Claude 3.5 Sonnet (Anthropic)</option>
                                            <option value="gemini-1.5-pro">Gemini 1.5 Pro (Google)</option>
                                            <option value="gemma2:2b">Gemma 2 2b (Local Ollama)</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-muted-foreground mb-1.5 uppercase tracking-wider">Fallback Model</label>
                                        <select
                                            value={freightConfig.fallback_ai_model}
                                            onChange={(e) => setFreightConfig({ ...freightConfig, fallback_ai_model: e.target.value })}
                                            className="w-full px-3 py-2 text-sm bg-white/5 border border-white/10 rounded-lg text-foreground outline-none focus:ring-2 focus:ring-primary/50 transition-all"
                                        >
                                            <option value="claude-3-5-sonnet">Claude 3.5 Sonnet (Anthropic)</option>
                                            <option value="gpt-4o">GPT-4o (OpenAI)</option>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center">
                                            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Extraction Threshold</label>
                                            <span className="text-[10px] font-mono text-primary font-bold">{Math.round(freightConfig.extraction_confidence_threshold * 100)}%</span>
                                        </div>
                                        <input
                                            type="range" min="0" max="1" step="0.05"
                                            value={freightConfig.extraction_confidence_threshold}
                                            onChange={(e) => setFreightConfig({ ...freightConfig, extraction_confidence_threshold: parseFloat(e.target.value) })}
                                            className="w-full accent-primary h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center">
                                            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Quarantine Threshold</label>
                                            <span className="text-[10px] font-mono text-amber-500 font-bold">{Math.round(freightConfig.quarantine_threshold * 100)}%</span>
                                        </div>
                                        <input
                                            type="range" min="0" max="0.5" step="0.05"
                                            value={freightConfig.quarantine_threshold}
                                            onChange={(e) => setFreightConfig({ ...freightConfig, quarantine_threshold: parseFloat(e.target.value) })}
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-muted-foreground mb-1.5 uppercase tracking-wider">Max Body Context (Chars)</label>
                                    <input
                                        type="number"
                                        value={freightConfig.max_email_body_chars_for_ai}
                                        onChange={(e) => setFreightConfig({ ...freightConfig, max_email_body_chars_for_ai: parseInt(e.target.value) || 8000 })}
                                        className="w-full px-3 py-2 text-sm bg-white/5 border border-white/10 rounded-lg text-foreground outline-none focus:ring-2 focus:ring-primary/50 transition-all font-mono"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                                <p className="text-xs text-muted-foreground">AI-assisted extraction is currently disabled. All processing will be strictly deterministic.</p>
                            </div>
                        )}
                    </motion.section>
                )}
