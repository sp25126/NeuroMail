...
export class ToolRegistry {
    ...
    private registerBuiltInTools() {
        ...
        this.registerTool({
            id: "context.reply_current",
            name: "reply_current_email",
            description: "Reply to the currently open email thread",
            parameters: [
                { name: "body", type: "string", description: "Email reply body text", required: true },
            ],
            returnType: "ComposeState",
            category: "mail",
            permissions: ["mail.compose"],
            ...
            if (!context.currentThread) {
                throw new Error("No thread currently open");
            }
            const sender = context.currentThread.participants?.[0] || "";
            const subject = context.currentThread.subject || "";
